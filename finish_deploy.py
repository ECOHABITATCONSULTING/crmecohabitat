#!/usr/bin/env python3
"""
Script pour FINIR le déploiement - Le repo est déjà cloné
On finit juste l'installation
"""
...
print("🚀 FINALISATION DÉPLOIEMENT CRM")
...
try:
    ...
    # CRITIQUE: Upgrade Node.js à v20
    print("\n🔧 UPGRADE NODE.JS v12 → v20 (REQUIS!)")
    exec_cmd(ssh,
        "curl -fsSL https://deb.nodesource.com/setup_20.x | sudo -E bash -",
        "ÉTAPE 1/8: Ajout repo Node.js 20")

    exec_cmd(ssh,
        "sudo apt install -y nodejs",
        "Installation Node.js 20")

    exec_cmd(ssh, "node --version", "Vérification Node.js")
    exec_cmd(ssh, "npm --version", "Vérification npm")

    # Fix permissions
    exec_cmd(ssh, f"sudo chown -R {VPS_USER}:{VPS_USER} {PROJECT_DIR}", "ÉTAPE 2/8: Fix permissions")
...
"""
    exec_cmd(ssh, f"cd {PROJECT_DIR}/backend && echo '{backend_env}' > .env", "ÉTAPE 3/8: Config backend .env")
    ...
    exec_cmd(ssh, f"cd {PROJECT_DIR}/frontend && echo '{frontend_env}' > .env", "ÉTAPE 4/8: Config frontend .env")
    exec_cmd(ssh, f"cd {PROJECT_DIR}/frontend && npm install", "Install frontend deps")
    exec_cmd(ssh, f"cd {PROJECT_DIR}/frontend && npm run build", "ÉTAPE 5/8: Build frontend")
...
"""
    ...
    # PM2
    exec_cmd(ssh, "pm2 delete all || true", "ÉTAPE 7/8: Clean PM2")
    exec_cmd(ssh, f"cd {PROJECT_DIR}/backend && pm2 start src/server.js --name crm-backend", "Start PM2")
    exec_cmd(ssh, "pm2 save", "Save PM2")
    exec_cmd(ssh, "pm2 list", "ÉTAPE 8/8: Status PM2")
...
🎯 PRÊT À TESTER LES 11 MODIFICATIONS !
    """)
